...
namespace Experior.Catalog.Qubiqa.TransferCar.Assemblies
{
    public class TransferCar : Assembly
    {
        ...
        public TransferCar(TransferCarInfo info) : base(info)
        {
            ...
            car1Transport = new StraightTransportSection(carColor, 1.5f, carHeight, 0.95f);
            Add(car1Transport);
            car1Transport.LocalYaw = (float)Math.PI / 2;
            car1Transport.Route.Motor.Speed = conveyorSpeed;

            rail1 = new Cube(Color.Gray, transport.Length, 0.02f, 0.02f);
            Add(rail1);
            rail1.LocalPosition = new Vector3(transport.Length / 2, 0, car1Transport.Length / 4);
            rail2 = new Cube(Color.Gray, transport.Length, 0.02f, 0.02f);
            Add(rail2);
            rail2.LocalPosition = new Vector3(transport.Length / 2, 0, -car1Transport.Length / 4);
